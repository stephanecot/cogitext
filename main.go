...
// cogit — le contexte partagé entre plusieurs utilisateurs, porté par la branche
...
func die(f string, a ...any)  { warn("cogit : "+f, a...); os.Exit(1) }
...
func positional() []string {
	...
	for i, a := range args {
		...
		if strings.HasPrefix(a, "--") {
			if i+1 < len(args) && !strings.HasPrefix(args[i+1], "--") && a != "--offline" && a != "--no-push" {
				skip = true
			}
			continue
		}
...
func resolveRoot(explicit string) string {
	...
	wd, _ := os.Getwd()
	// On remonte jusqu'au dépôt : cogit peut être appelé depuis n'importe quel
...
const help = `cogit — contexte partagé sur la branche orpheline « context »

  init                    créer ou rejoindre la branche, monter .ctx, câbler les hooks
  ...
  debug on|off|tail|clear tracer les interactions dans .claude/cache/ctx/ctx.log
...
func main() {
	...
	case "hook-start", "hook-prompt", "hook-guard":
		...
		defer func() { _ = recover() }() // une panique de ctx ne coûte jamais un tour
		switch cmd {
		case "hook-start":
			hookStart(root, in)
		case "hook-prompt":
...
func cmdInit() {
	...
	if !HasBranch(root) {
		...
		if RevParse(root, RemoteRef) != "" {
			...
			say("cogit : branche `context` récupérée depuis origin.")
		} else {
			if !fetched.OK && !strings.Contains(fetched.Err, "couldn't find remote ref") {
				say("cogit : remote injoignable — création locale, le push se fera plus tard.")
			}
			r := runGit([]string{"-c", "user.name=ctx", "-c", "user.email=ctx@local",
				"commit-tree", "-m", "cogit: root", emptyTree},
			...
			say("cogit : branche `context` créée (racine déterministe %s).", short(r.Out))
		}
	}

	wt := CogitDir(root)
	if _, err := os.Stat(filepath.Join(wt, ".git")); err == nil {
		say("cogit : worktree `.cogit` déjà en place.")
	} else if entries, err := os.ReadDir(wt); err == nil && len(entries) > 0 {
		die("`.cogit` existe et n'est pas un worktree — je ne supprime rien, à toi de voir")
	} else {
		...
		say("cogit : worktree monté sur `.cogit`.")
	}
	...
	ensureIgnored()
	writeSettings()
	say("cogit : prêt. `cogit add decision` pour enregistrer une règle.")
}

// .cogit et le cache dérivé ne doivent jamais entrer dans un commit du projet.
func ensureIgnored() {
	gi := filepath.Join(root, ".gitignore")
	b, _ := os.ReadFile(gi)
	text := string(b)
	var missing []string
	for _, line := range []string{"/.cogit/", "/.claude/cache/"} {
		found := false
	...
	if err := os.WriteFile(gi, []byte(out), 0o644); err == nil {
		say("cogit : %s ajouté(s) à .gitignore.", strings.Join(missing, " et "))
	}
}
...
func writeSettings() {
	...
	if err := os.WriteFile(p, append(b, '\n'), 0o644); err == nil {
		say("cogit : hooks câblés dans .claude/settings.local.json (%s/%s).", runtime.GOOS, runtime.GOARCH)
	}
}
...
func pushCtx() (bool, bool, string) {
	...
	for i := 0; i < 3; i++ {
		...
		if !m.OK {
			git(wt, "merge", "--abort")
			return false, true, "conflit sur la branche context — `cogit sync` puis réessaie"
		}
	}
...
func publish(what string) {
	...
	default:
		say("cogit : commit local gardé, publication différée (%s).", why)
	}
}

func cmdPush() {
	if !WorktreeReady(root) {
		die("worktree absent — lance `cogit init`")
	}
	ahead := git(root, "rev-list", "--count", RemoteRef+".."+Ref)
	if ahead.OK && ahead.Out == "0" {
		say("cogit : rien à publier.")
		return
	}
	say("cogit : %s commit(s) à publier.", ahead.Out)
	publish("")
}
...
func cmdAdd() {
	p := positional()
	if len(p) == 0 {
		die("usage : cogit add decision|fact|note  (JSON sur stdin)")
	}
	kind := p[0]
	...
	if hasFlag("no-push") {
		say("cogit : commit local, non publié (`cogit push` pour le lot).")
		return
	}
	publish(rel)
}
...
func cmdSync() {
	if !HasBranch(root) {
		die("branche `context` absente — lance `cogit init`")
	}
	cfg := LoadConfig(root)
	before := ""
	...
	if !hasFlag("offline") {
		if f := FetchContext(root, 15*time.Second); !f.OK {
			say("cogit : remote injoignable (%s) — synchronisation locale.", firstLine(f.Err))
		}
	}
		...
		if anc.Code == 0 {
			...
			if WorktreeReady(root) {
				r = runGit([]string{"merge", "--ff-only", "-q", RemoteRef},
					gitOpts{Dir: CogitDir(root), Timeout: 10 * time.Second})
			} else {
				r = git(root, "update-ref", Ref, remote)
			}
			if !r.OK {
				say("cogit : avance rapide impossible — %s", firstLine(r.Err))
			}
		} else {
			say("cogit : la branche locale a divergé de origin — `cogit doctor` pour le détail.")
		}
	}
	...
	if len(changes) == 0 {
		say("cogit : à jour (%s, %d décisions).", short(h.Gate), h.N.Decisions)
	} else {
		say("cogit : %d entrée(s) depuis ton dernier point de contrôle —", len(changes))
		for i, c := range changes {
			if i >= cfg.DeltaMaxEntries {
				say("  • +%d de plus — cogit list decisions", len(changes)-cfg.DeltaMaxEntries)
				break
			}
			body := c.Rule
			if body == "" {
	...
	if sid != "" {
		...
		say("cogit : session ré-épinglée sur %s. Écritures débloquées.", short(h.Gate))
	}
}
...
func cmdFind() {
	...
	if len(q) == 0 {
		die("usage : cogit find \"<mots-clés>\"")
	}
	...
	if len(hits) == 0 {
		say("cogit : aucune entrée ne correspond.")
		return
	}
...
func cmdShow() {
	p := positional()
	if len(p) == 0 {
		die("usage : cogit show <id>")
	}
	for _, ext := range []string{".yaml", ".yml", ".md"} {
		if b, err := os.ReadFile(filepath.Join(CogitDir(root), p[0]+ext)); err == nil {
			os.Stdout.Write(b)
			return
		}
	}
...
func cmdList() {
	...
	if len(rows) == 0 {
		say("cogit : aucune entrée de type « %s ».", kind)
		return
	}
...
func cmdDoctor() {
	...
	if len(expired) > 0 {
		say("cogit : %d fait(s) expiré(s), hors du brief — %s", len(expired), strings.Join(expired, ", "))
	}
	...
	say("cogit : %s/%s · %s%s", runtime.GOOS, runtime.GOARCH, gitv, extra)

	attrs, _ := os.ReadFile(filepath.Join(CogitDir(root), ".gitattributes"))
	...
	if !WorktreeReady(root) {
		problems = append(problems, "worktree `.cogit` absent — `cogit init`")
	}

	say("cogit : brief %d/%d octets · %d décisions · %d faits · %d notes",
		size, cfg.BriefMaxBytes, h.N.Decisions, h.N.Facts, h.N.Notes)
	if len(problems) == 0 {
		say("cogit : rien à signaler.")
		return
	}
...
func cmdCompact() {
	r := runGit([]string{"gc", "--prune=now", "--quiet"},
		gitOpts{Dir: CogitDir(root), Timeout: 120 * time.Second})
	if r.OK {
		say("cogit : base compactée.")
	} else {
		say("cogit : compactage impossible — %s", firstLine(r.Err))
	}
}
...
func cmdDebug() {
	...
	case "on":
		...
		_ = os.WriteFile(marker, []byte("ctx debug on — "+time.Now().Format(time.RFC3339)+"\n"), 0o644)
		rel, _ := filepath.Rel(root, logf)
		say("cogit : traçage activé → %s", filepath.ToSlash(rel))
	case "off":
		_ = os.Remove(marker)
		say("cogit : traçage désactivé.")
	case "clear":
		_ = os.Remove(logf)
		_ = os.Remove(logf + ".1")
		say("cogit : journal effacé.")
	case "tail":
		n := 30
		if v := flagVal("n"); v != "" {
			fmt.Sscanf(v, "%d", &n)
		...
		if err != nil {
			say("cogit : aucun journal — `cogit debug on` puis rejoue.")
			return
		}
		lines := []string{}
	...
	default:
		...
		say("cogit : traçage %s%s", state, size)
		say("  cogit debug on|off|tail|clear")
	}
}
