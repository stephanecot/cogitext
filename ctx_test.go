...
func TestBriefStaysUnderCapAt500Entries(t *testing.T) {
	...
	if !strings.Contains(brief, "more — cogit find") {
...
func seedStale(t *testing.T, root string) {
	t.Helper()
	cache := filepath.Join(root, ".claude", "cache", "cogit")
...
func TestGuardDisabledSwitch(t *testing.T) {
	...
	seedStale(t, root)
	_ = os.WriteFile(filepath.Join(root, ".claude", "cache", "cogit", "DISABLED"), []byte("test"), 0o644)
