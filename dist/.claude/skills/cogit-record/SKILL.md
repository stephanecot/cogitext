...
**Delegate to the `cogit-curator` agent** when the material is a document, a page
...
whose language is not guaranteed, and a half-translated corpus breaks `find`, since
one half stops answering the other half's keywords. `cogit add` refuses French prose
at write time, so you find out immediately rather than six months later.
...
- **Never `git add -A` inside `.cogit`,** and never commit there by hand — `cogit add`
  stages the exact two paths it wrote.
