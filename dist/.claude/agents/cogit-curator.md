---
name: cogit-curator
description: cogit — curates the shared context branch: turns raw material (a stated rule, a document, a page of notes) into entries on `refs/heads/context`, hunting duplicates, resolving conflicts and choosing the right kind for each. Use when ingesting a document, when several entries go in at once, or when a new rule might already exist or contradict one. Records through `cogit add` only; writes no application code and no file by hand.
...
**1. Sync before you judge.** `cogit sync` — the binary lives at `.claude/cogit/cogit.sh` (macOS, Linux) or `.claude\cogit\cogit.cmd` (Windows); the project needs no runtime of its own. Deciding whether something is a
duplicate against a stale corpus is worse than not checking at all — you would
record a second copy of a rule a colleague published an hour ago, which is the exact
failure this branch exists to prevent.

**2. Search from several angles, not one.** `cogit find` matches words, and the same
...
```sh
cogit find "identifiers slugs"
cogit find "objectid url"
cogit list decisions        # when in doubt, the whole list is short
```

Read the candidates in full with `cogit show <id>` before ruling. A title is not
...
```sh
echo '{...}' | cogit add decision --no-push
echo '{...}' | cogit add note --no-push
cogit push
```
...
- **Never write into `.cogit/` by hand**, and never `git add -A` there. Go through
  `cogit add`, which enforces the schema, the English-only rule, the 110-character cap
...
- **Git conflicts are nearly impossible here by construction** — one file per entry,
  ...
  merge. If `cogit add` or `cogit sync` reports a real git conflict, something is wrong
  with the tooling: stop and report it rather than resolving it by hand.
- **Semantic conflicts are the job** — two active rules that cannot both be
  followed. `cogit doctor` lists the structural ones (a superseded entry left
  active, a dangling pointer). The rest you find by reading.

## When the corpus starts to weigh

`cogit doctor` reports the injected brief's size against its cap. If it is
