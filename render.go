...
const staleness = "## Staleness\n" +
	...
	"or a fact, Edit/Write are denied until `cogit sync`, which prints only the delta.\n" +
	"Notes and journal entries never block anyone.\n" +
	"Nothing above is the whole corpus: `cogit find` covers 100% of it, including what\n" +
	"was truncated here."

func reaching(n Counts) string {
	return "## Reaching the rest\n" +
		"one entry in full          cogit show <id>\n" +
		"search everything          ctx find \"<keywords>\"\n" +
		fmt.Sprintf("notes (%d) and journal (%d), never auto-loaded   cogit list notes\n", n.Notes, n.Journal) +
		"record a new one           cogit add decision|fact|note"
}
...
func RenderBrief(c Corpus, h *Head, cfg Config) string {
	...
	header := fmt.Sprintf("# Shared context (cogit) — %s · %d decisions · %d facts · %d notes · %d journal",
	...
	// le plafond. Rien n'est perdu : la queue « +N more » et `ctx find` gardent 100 %
	...
	if kept < len(decisions) {
		lines = append(lines, fmt.Sprintf("+%d more — cogit find \"<keywords>\"", len(decisions)-kept))
	}
...
func RenderDelta(changes []Change, max int) string {
	...
	fmt.Fprintf(&sb, "cogit: %d new %s since your last checkpoint.\n", n, plural)
	for _, c := range head {
		t := c.Title
		if t == "" {
			t = "(untitled)"
	...
	if n > max {
		fmt.Fprintf(&sb, "  • +%d more — cogit sync\n", n-max)
	}
