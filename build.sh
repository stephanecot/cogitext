#!/usr/bin/env bash
# Reconstruit les binaires cogitex pour toutes les plateformes cibles, dans le
...
set -euo pipefail
cd "$(dirname "$0")"
OUT="dist/.claude/cogitex/bin"
mkdir -p "$OUT"
for t in darwin/arm64 darwin/amd64 linux/amd64 linux/arm64 windows/amd64; do
  os="${t%/*}"; arch="${t#*/}"; ext=""
  [ "$os" = windows ] && ext=".exe"
  CGO_ENABLED=0 GOOS="$os" GOARCH="$arch" \
    go build -trimpath -ldflags="-s -w" -o "$OUT/cogitex-$os-$arch$ext" ./cmd/cogitex
  printf '  %-22s %s\n' "cogitex-$os-$arch$ext" "$(du -h "$OUT/cogitex-$os-$arch$ext" | cut -f1)"
done
